...
def _username_map(session: Session) -> dict[int, str]:
    """id → отображаемое имя для обогащения ответов."""
    return {u.id: (u.full_name or u.username) for u in session.exec(select(models.User)).all()}
...
def list_entries(session: Session = Depends(get_session)):
    ...
    for i in items:
        d = i.model_dump()
        d["created_by_username"] = users.get(i.created_by_user_id) if i.created_by_user_id else None
        result.append(d)
    return result
